...
`;
  ...
  useEffect(() => {
    ...
    function onUserActivity() {
      ...
      sessionTimeout = setTimeout(endSessionIfInactive, 5 * 60 * 1000); // 5 minutes
    }

    async function endSessionIfInactive() {
      const now = new Date();
      if (now.getTime() - lastActive.getTime() >= 5 * 60 * 1000) {
        await saveSession(sessionStart, lastActive);
    ...
    // Save session when component unmounts or user leaves
    async function cleanup() {
      clearTimeout(sessionTimeout);
      const now = new Date();
      // Only save if the session was at least 1 minute long
      if (now.getTime() - sessionStart.getTime() >= 60 * 1000) {
        await saveSession(sessionStart, now);
      }
    }
    ...
    sessionTimeout = setTimeout(endSessionIfInactive, 5 * 60 * 1000);
    ...
    return () => {
      ...
      clearTimeout(sessionTimeout);
      // Call cleanup directly when component unmounts
      cleanup();
    };
  }, [user]);
